...
uint8_t g_pwm_buffer[2][0xC0] = {0};


#define RGB_3733_NUM 2
...
static const uint8_t RGB_3733_ADDR[2][3] = {
    {0x20, 0x00, 0x10}, 
    {0x10, 0x00, 0x20}, 
};
...
void IS31FL3733_set_color(uint8_t data, uint8_t red, uint8_t green, uint8_t blue)
{
    ...
    if (index.num  < RGB_3733_NUM) 
    {
        g_pwm_buffer[index.num][RGB_3733_ADDR[index.num][0]+index.sw_lr*0x30 + index.offset ] = red;
        g_pwm_buffer[index.num][RGB_3733_ADDR[index.num][1]+index.sw_lr*0x30 + index.offset ] = green;
        g_pwm_buffer[index.num][RGB_3733_ADDR[index.num][2]+index.sw_lr*0x30 + index.offset ] = blue;
...
void IS31FL3733_update_pwm_buffers_all()
{
    if (g_pwm_buffer_update_required) {
        ...
        IS31FL3733_write_pwm_buffer(0x5F, g_pwm_buffer[1]);
    }
}