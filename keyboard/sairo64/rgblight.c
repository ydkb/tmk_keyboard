...
#define SRGBLED_NUM 64
#define rgblight_timer_init() do { DDRB |= (1<<PB1 | 1<<PB0);} while(0)
#define rgblight_timer_enable() do { PORTB |= (1<<PB1); PORTB &= ~(1<<PB0);} while(0)
#define rgblight_timer_disable() do { PORTB &= ~(1<<PB1);  PORTB |= (1<<PB0);} while(0)
#define rgblight_timer_enabled (PORTB&(1<<PB1))
...
const uint8_t SLED_MATRIX_MASK[MATRIX_ROWS][MATRIX_COLS]  = 
{
    {0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D},
    {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D},
    {0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B, 0x2C, 0x40},
    {0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D},
    {0x3E, 0x3F, 0x0F, 0x40, 0x40, 0x0E, 0x40, 0x40, 0x40, 0x1E, 0x1F, 0x2F, 0x2E, 0x2D},
};
...
void rgblight_effect_snake_auto(uint8_t interval)
{
    ...
    if (snake_control[5][0] == snake_control[6][0] && snake_control[5][1] == snake_control[6][1]) {
        ...
        wave_col_l[0] = snake_control[6][1] = rand()%(MATRIX_COLS-1);
    ...
    if (snake_control[5][1] != snake_control[6][1]) {
        ...
        for (i=0; i<3; i++) {
            if (snake_control[5][0] == snake_control[i][0] && snake_control[5][1] + increament == snake_control[i][1]) {
                if (snake_control[5][0] + 1 < 5) snake_control[5][0]++;
                ...
                if (j != random_delay1 && j != random_delay2) {
                    if (--fading_single_key[j] == 0) { 
                        if (--horse_race_remain) {
                            ...
                            if (horse_race_remain>1) led[9-2*horse_race_remain] = sled[SRGBLED_NUM];
                        }
                    }
                }
            } 
